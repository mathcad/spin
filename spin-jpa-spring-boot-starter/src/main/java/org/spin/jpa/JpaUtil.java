...
public abstract class JpaUtil {
     ...
     * 创建Linq
     *
     * @param resultClass 查询结果类
     * @param <T>         查询结果类范型
     * @return Linq
     */
    public static <PK extends Serializable, E extends IEntity<PK, E>, T extends IVo<PK, E>> Linq linq(Class<? extends T> resultClass) {
        ...
        if (null == domainClass) {
            throw new SimplifiedException("Result类型没有正确关联到领域模型, 无法构建查询");
        }
     ...
     * @return Linq
     */
    public static <T> Linq linq(Class<T> resultClass, EntityManager entityManager) {
        Type[] genericInterfaces = resultClass.getGenericInterfaces();
        Class<?> domainClass = null;
        for (Type genericInterface : genericInterfaces) {
            if (genericInterface instanceof ParameterizedType) {
