...
public class IdCardUtils {
    ...
    private static final DateTimeFormatter BIRTHDAY_PATTERN = DateTimeFormatter.ofPattern("yyyyMMdd");
    ...
    public static String convert15To18(String idCard) {
        ...
        if (StringUtils.isInteger(idCard)) {
            // 获取出生年月日
            String birthday = idCard.substring(6, 12);
            try {
                LocalDate birthDate = LocalDate.parse("19" + birthday, BIRTHDAY_PATTERN);
                // 获取出生年(完全表现形式,如：2010)
    ...
    public static boolean isValid(String idCard) {
        ...
        switch (length) {
            ...
            case 10: {// 10位身份证，港澳台地区
                String[] cardVal = isValid10(idCard);
     ...
     * @return 是否有效的18位身份证
     */
    public static boolean isValid18(String idCard) {
        ...
        try {
            LocalDate parse = LocalDate.parse(birthday, BIRTHDAY_PATTERN);

            if (parse.getYear() < 1900 || parse.isAfter(LocalDate.now())) {
                return false;
            }
        } catch (DateTimeParseException ignore) {
            return false;
        ...
        if (StringUtils.isInteger(code17)) {
            // 获取校验位
     ...
     * @return 是否合法
     */
    public static boolean isValid15(String idCard) {
        ...
        if (StringUtils.isInteger(idCard)) {
            ...
            try {
                LocalDate parse = LocalDate.parse("19" + idCard.substring(6, 12), BIRTHDAY_PATTERN);
                return parse.getYear() >= 1900 && !parse.isAfter(LocalDate.now());
            } catch (DateTimeParseException ignore) {
                return false;
            }
        } else {
     ...
     * </p>
     */
    public static String[] isValid10(String idCard) {
        ...
        if (idCard.matches("^[a-zA-Z][0-9]{9}$")) { // 台湾
            ...
            info[2] = isValidTw(idCard) ? "true" : "false";
            ...
            info[0] = "香港";
            info[1] = "N";
            info[2] = isValidHk(idCard) ? "true" : "false";
        } else {
            return null;
        }
        return info;
     ...
     * @return 验证码是否符合
     */
    public static boolean isValidTw(String idCard) {
        if (StringUtils.isBlank(idCard)) {
            return false;
        }
     ...
     * @return 验证码是否符合
     */
    public static boolean isValidHk(String idCard) {
        String card = idCard.replaceAll("[()]", "");
        int sum;
        if (card.length() == 9) {
    ...
    public static LocalDate getBirthDate(String idCard) {
        ...
        return null == birthByIdCard ? null : LocalDate.parse(birthByIdCard, BIRTHDAY_PATTERN);
    }
    ...
    public static int getAgeByIdCard(String idCard, LocalDate dateToCompare) {
        ...
        return null == birth ? -1 : birth.until(dateToCompare).getYears();
    }
    ...
    public static String getProvinceByIdCard(String idCard) {
        ...
        if (len == CHINA_ID_MIN_LENGTH || len == CHINA_ID_MAX_LENGTH) {
            ...
            return PROVINCE_CODES.get(sProvinNum);
        }
        return null;
    }
