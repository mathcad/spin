...
//        parse = UserAgentUtil.parse("PostmanRuntime/7.17.1");
    ...
    @Test
    void testLunar() {
        LunarDate lunarDate = new LunarDate(LocalDate.of(2033, 11, 1));
        System.out.println(lunarDate);
        lunarDate = new LunarDate(LocalDate.now().plusDays(2));
        System.out.println(lunarDate);
    }
    ...
    public static class LunarDate {
         ...
         * 月份
         */
        private static final String[] CHINESE_MOUNTHS = {"正", "二", "三", "四", "五", "六", "七", "八", "九", "十", "冬", "腊"};
         ...
         * 节气
         */
        private static final String[] LUNAR_HOLDAY_NAME = {
            "小寒", "大寒", "立春", "雨水", "惊蛰", "春分",
            "清明", "谷雨", "立夏", "小满", "芒种", "夏至",
            "小暑", "大暑", "立秋", "处暑", "白露", "秋分",
            "寒露", "霜降", "立冬", "小雪", "大雪", "冬至"
        ...
        public LunarDate(LocalDate date) {
            // 求出和1900年1月31日相差的天数
            long offset = BASE_DATE.until(date, ChronoUnit.DAYS);
        ...
        public String getChineseLeap() {
            return (leap ? "闰" : "");
        }

        public String getChineseMonth() {// 返回阴历的月份
            return CHINESE_MOUNTHS[month - 1];
        }

        public String getChineseDay() {// 返回阴历的天
            return chineseDay(day);
        }

        public String getBigOrSmall() {// 返回的月份的大或小
        ...
        public String toString() {
            return String.format("%s(%s)年 %s%s月%s", getGanzhi(), getZodiac(), getChineseLeap(), getChineseMonth(), getChineseDay());
        }
        ...
        private String ganzhi(int num) {
            return (TIAN_GAN[num % 10] + DI_ZHI[num % 12]);
        }

        private String chineseDay(int day) {
            int n = day % 10;
            if (day == 10) {
                return "初十";
            } else if (day == 20) {
...
class SolarTerm {
    ...
    // ========角度变换===============
    // 每弧度的角秒数
    private static final double rad = 180D * 3600D / Math.PI;
    // 每弧度的角度数
    private static final double RAD = 180D / Math.PI;

    // 一回归年的平均天数
    private static final double DAYS_PER_YEAR = 365d + (5D * 3600D + 48D * 60D + 46D) / 86400D;

    // ================日历计算===============
    private static final double J2000 = 2451545D; // 2000年前儒略日数(2000-1-1 12:00:00格林威治平时)

    // =========黄赤交角及黄赤坐标变换===========
    // 黄赤交角系数表
    private static final double[] hcjjB = {84381.448D, -46.8150D, -0.00059D, 0.001813D};
    // Date黄道上的岁差p
    private static final double[] preceB = {0D, 50287.92262D, 111.24406D, 0.07699D, -0.23479D, -0.00178D, 0.00018D, 0.00001D};
    ...
    private static final double[] dt_at = {
        -4000D, 108371.7D, -13036.80D, 392.000D, 0.0000D,
        -500D, 17201.0D, -627.82D, 16.170D, -0.3413D,
        -150D, 12200.6D, -346.41D, 5.403D, -0.1593D,
        150D, 9113.8D, -328.13D, -1.647D, 0.0377D,
        500D, 5707.5D, -391.41D, 0.915D, 0.3145D,
        900D, 2203.4D, -283.45D, 13.034D, -0.1778D,
        1300D, 490.1D, -57.35D, 2.085D, -0.0072D,
        1600D, 120.0D, -9.81D, -1.532D, 0.1403D,
        1700D, 10.2D, -0.91D, 0.510D, -0.0370D,
        1800D, 13.4D, -0.72D, 0.202D, -0.0193D,
        1830D, 7.8D, -1.81D, 0.416D, -0.0247D,
        1860D, 8.3D, -0.13D, -0.406D, 0.0292D,
        1880D, -5.4D, 0.32D, -0.183D, 0.0173D,
        1900D, -2.3D, 2.06D, 0.169D, -0.0135D,
        1920D, 21.2D, 1.69D, -0.304D, 0.0167D,
        1940D, 24.2D, 1.22D, -0.064D, 0.0031D,
        1960D, 33.2D, 0.51D, 0.231D, -0.0109D,
        1980D, 51.0D, 1.29D, -0.026D, 0.0032D,
        2000D, 63.87D, 0.1D, 0.0000D, 0.0000D,
        2005D, 64.70D, 0.4D, 0.0000D, 0.0000D,
        2015D, 69D,
//        2150D, 279.4D, 732.95D, 429.579D, 0.0158D,
//        6000D
    };

    public SolarTerm(LocalDateTime dateTime) {
    ...
    // 取整数部分
    public static double truncateInt(double v) {
        v = Math.floor(v);
        return v < 0 ? (v + 1) : v;
    }

    // 对超过0-2PI的角度转为0-2PI
    public static double normalizeRad(double v) {
        v = v % (2 * Math.PI);
        return v < 0 ? (v + 2 * Math.PI) : v;
    }

    public double dt_ext(double y, double jsd) {
    ...
    public double deltatT2(double jd) {
        return deltatT(jd / DAYS_PER_YEAR + 2000D) / 86400.0D;
    }

    // 公历转儒略日,UTC=1表示原日期是UTC
    public double toJD(boolean utc) {
        double y = this.year; // 取出年月
        double m = this.month;
        double n = 0;
        ...
        if (this.year * 372 + this.month * 31 + this.day >= 588829) {
            ...
            n = truncateInt(y / 100);
            n = 2 - n + truncateInt(n / 4);// 加百年闰
        }

        n += truncateInt(DAYS_PER_YEAR * (y + 4716)); // 加上年引起的偏移日数
        n += truncateInt(30.6 * (m + 1)) + this.day; // 加上月引起的偏移日数及日偏移数
        n += ((this.second / 60 + this.minute) / 60 + this.hour) / 24 - 1524.5;
        if (utc)
    ...
    public void setFromJD(double jd, boolean UTC) {
        ...
        // 取得日数的整数部份A及小数部分F
        double A = truncateInt(jd);
        double F = jd - A;
        double D;

        if (A > 2299161) {
            D = truncateInt((A - 1867216.25) / DAYS_PER_YEAR);
            A += 1 + D - truncateInt(D / 4);
        }
        A += 1524; // 向前移4年零2个月
        this.year = truncateInt((A - 122.1) / DAYS_PER_YEAR);// 年
        D = A - truncateInt(DAYS_PER_YEAR * this.year); // 去除整年日数后余下日数
        this.month = truncateInt(D / 30.6001); // 月数
        this.day = D - truncateInt(this.month * 30.6001);// 去除整月日数后余下日数
        this.year -= 4716;
        this.month--;
        if (this.month > 12)
            this.month -= 12;
        ...
        // 日的小数转为时分秒
        F *= 24;
        this.hour = truncateInt(F);
        F -= this.hour;
        F *= 60;
        this.minute = truncateInt(F);
        F -= this.minute;
        F *= 60;
        this.second = F;
    }
    ...
    // 日期转为串
    public String toStr() {
        String Y = "     " + (int) this.year;
        String M = "0" + (int) this.month;
        String D = "0" + (int) this.day;
...
//        double Y = this.year, M = this.month, D = this.day, h = this.hour, m = this.minute, s = this.second; // 备份原来的数据
//
...
//    }
    ...
    public static void HCconv(double[] JW, double E) {
        // 黄道赤道坐标变换,赤到黄E取负
        double HJ = normalizeRad(JW[0]), HW = JW[1];
    ...
    public static void addPrece(double jd, double[] zb) {
        ...
        zb[0] = normalizeRad(zb[0] + (v + 2.9965 * t1) / rad);
    }

    // ===============光行差==================
    private static final double[] GXC_e = {0.016708634D, -0.000042037D, -0.0000001267D}; // 离心率
    private static final double[] GXC_p = {102.93735 / RAD, 1.71946 / RAD,
        0.00046 / RAD}; // 近点
    private static final double[] GXC_l = {280.4664567 / RAD,
        36000.76982779 / RAD, 0.0003032028 / RAD, 1 / 49931000D / RAD,
        -1 / 153000000D / RAD}; // 太平黄经
    ...
    public double[] sunCal2(double jd) {
        ...
        sun[1] = -sun[1]; // 计算太阳真位置
        ZD d = nutation(jd);
        sun[0] = normalizeRad(sun[0] + d.Lon); // 补章动
    ...
    public double[] moonCal(double jd) {
        ...
        llr[0] = llr[0] + M1n[0] + M1n[1] * t1 + M1n[2] * t2 + M1n[3] * t3
            + M1n[4] * t4;
        llr[0] = normalizeRad(llr[0]); // 地心Date黄道原点坐标(不含岁差)
        addPrece(jd, llr); // 补岁差
        return llr;
    }
    ...
    public double[] moonCal2(double jd) {
        ...
        ZD d = nutation(jd);
        moon[0] = normalizeRad(moon[0] + d.Lon); // 补章动
        return moon;
    }
    ...
    public double jiaoCai(int lx, double t, double jiao) {
        ...
        if (lx == 0) {
            ZD d = nutation(t);
            sun[0] += d.Lon; // 补黄经章动
            return normalizeRad(jiao - sun[0]);
        }
        double[] moon = moonCal(t); // 日月角差与章动无关
        return normalizeRad(jiao - (moon[0] - sun[0]));
    }
     ...
     * </pre>
     *
     * @param t1   是J2000起算儒略日数
     * @param jiao 已知角度求时间(t)
     * @param lx   类型 lx=0是太阳黄经达某角度的时刻计算(用于节气计算), lx=1是日月角距达某角度的时刻计算(用于定朔望等)
     * @return 指定角度对应的时间
     */
    ...
    public void JQtest(int y) { // 节气使计算范例,y是年分,这是个测试函数
        double jd = DAYS_PER_YEAR * (y - 2000D), q;
        String s1, s2;
        for (int i = 0; i < 24; i++) {
    ...
    public void dingSuo(int y, double arc) { // 这是个测试函数
        double jd = DAYS_PER_YEAR * (y - 2000), q;
        String s1, s2;
        System.out.println("月份:世界时  原子时");
        for (int i = 0; i < 12; i++) {
            q = jiaoCal(jd + 29.5 * i, arc, 1) + J2000 + 8D / 24D; // 计算第i个节气(i=0是春风),结果转为北京时
            setFromJD(q, true);
            s1 = toStr(); // 将儒略日转成世界时
            setFromJD(q, false);
            s2 = toStr(); // 将儒略日转成日期格式(输出日期形式的力学时)
    ...
    public void paiYue(int y) { // 农历排月序计算,可定出农历
        ...
        // 从冬至开始,连续计算14个中气时刻
        int i;
        double t1 = DAYS_PER_YEAR * (y - 2000) - 50; // 农历年首始于前一年的冬至,为了节气中气一起算,取前年大雪之前
...
//            B[i] = Dint_dec(jq[i], 8, true);
        ...
        // 显示
        StringBuilder out = new StringBuilder("节气    手表时            中气    手表时            农历月   朔的手表时/r/n");
        for (i = 0; i < tot; i++) {
            int zm = (i * 2 + 18) % 24;
            int jm = (i * 2 + 17) % 24; // 中气名节气名
            setFromJD(jq[i] + J2000 + (double) 8 / 24, true);
            out.append(jqB[jm]).append(":").append(toStr()).append(" "); // 显示节气
            setFromJD(zq[i] + J2000 + (double) 8 / 24, true);
            out.append(jqB[zm]).append(":").append(toStr()).append(" "); // 显示中气
            setFromJD(hs[i] + J2000 + (double) 8 / 24, true);
            out.append(syn[i]).append(":").append(toStr()).append("/r/n"); // 显示日月合朔
        }
        System.out.println(out);
    }

    public static void main(String[] argv) {
        SolarTerm st = new SolarTerm(LocalDateTime.now());
        st.JQtest(2020);
        //st.paiYue(2500);
