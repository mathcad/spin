...
import org.zibra.io.convert.DefaultConverter;
...
public class Reader {
    ...
    public Reader(InputStream stream) {
        this(stream, ZibraMode.MemberMode, false);
    }

    public Reader(InputStream stream, boolean simple) {
        this(stream, ZibraMode.MemberMode, simple);
    }

    public Reader(InputStream stream, ZibraMode mode) {
        this(stream, mode, false);
    }

    public Reader(InputStream stream, ZibraMode mode, boolean simple) {
        this.stream = stream;
        this.mode = mode;
    ...
    public Reader(ByteBuffer buffer) {
        this(buffer, ZibraMode.MemberMode, false);
    }

    public Reader(ByteBuffer buffer, boolean simple) {
        this(buffer, ZibraMode.MemberMode, simple);
    }

    public Reader(ByteBuffer buffer, ZibraMode mode) {
        this(buffer, mode, false);
    }

    public Reader(ByteBuffer buffer, ZibraMode mode, boolean simple) {
        this(new ByteBufferStream(buffer).getInputStream(), mode, simple);
    }

    public Reader(byte[] bytes) {
        this(bytes, ZibraMode.MemberMode, false);
    }

    public Reader(byte[] bytes, boolean simple) {
        this(bytes, ZibraMode.MemberMode, simple);
    }

    public Reader(byte[] bytes, ZibraMode mode) {
        this(bytes, mode, false);
    }

    public Reader(byte[] bytes, ZibraMode mode, boolean simple) {
        this(new ByteArrayInputStream(bytes), mode, simple);
    }

    public final ZibraException unexpectedTag(int tag) {
        return unexpectedTag(tag, null);
    }

    public final ZibraException unexpectedTag(int tag, String expectTags) {
        if (tag == -1) {
            return new ZibraException("No byte found in stream");
        }
        else if (expectTags == null) {
            return new ZibraException("Unexpected serialize tag '" +
                                       (char)tag + "' in stream");
        }
        else {
            return new ZibraException("Tag '" + expectTags +
                                       "' expected, but '" + (char)tag +
                                       "' found in stream");
        }
    }

    public final void checkTag(int tag, int expectTag) throws ZibraException {
        if (tag != expectTag) {
    ...
    public final String tagToString(int tag) throws IOException {
        switch (tag) {
            ...
            case TagError: throw new ZibraException(readString());
