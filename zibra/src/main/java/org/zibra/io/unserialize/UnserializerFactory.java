...
import org.zibra.util.JdkVersion;
...
public final class UnserializerFactory {
    private final static ConcurrentHashMap<Class<?>, Unserializer> unserializers = new ConcurrentHashMap<>();
    static {
        ...
        if (JdkVersion.majorJavaVersion >= JdkVersion.JAVA_18) {
            try {
                Class.forName("org.zibra.io.unserialize.java8.UnserializerLoader");
            }
            catch (ClassNotFoundException ignored) {}
        }
    }

    public static Unserializer get(Class<?> type) {
    ...
    public static void register(Class<?> type, Unserializer unserializer) {
        unserializers.put(type, unserializer);
    }
