...
public final class Accessors {
    private final static ConcurrentHashMap<Type, LinkedCaseInsensitiveMap<String, MemberAccessor>> propertiesCache = new ConcurrentHashMap<>();
    private final static ConcurrentHashMap<Type, LinkedCaseInsensitiveMap<String, MemberAccessor>> membersCache = new ConcurrentHashMap<>();
    private final static ConcurrentHashMap<Type, LinkedCaseInsensitiveMap<String, MemberAccessor>> fieldsCache = new ConcurrentHashMap<>();
    ...
    public static Map<String, MemberAccessor> getMembers(Type type, ZibraMode mode) {
        Class<?> clazz = ClassUtil.toClass(type);
        return ((mode != ZibraMode.MemberMode) && Serializable.class.isAssignableFrom(clazz)) ?
                (mode == ZibraMode.FieldMode) ?
                        getFields(type) :
                        getProperties(type) :
                getMembers(type);
    }
