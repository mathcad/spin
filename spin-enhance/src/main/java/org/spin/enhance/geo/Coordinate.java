...
public class Coordinate implements Serializable {
     ...
     * 扁率
     */
    private static final double E = 0.00669342162296594323D;
    ...
    public static void main(String[] args) {
        ...
        // gps坐标
        gc = gpsToGcj02(31.35088889, 118.40396354);
        System.out.println(gc);
        System.out.println(gcj02ToBd09(gc.latitude, gc.longitude));
    ...
    public Coordinate(double latitude, double longitude) {
        this(CoordinateSystem.GPS, latitude, longitude);
    }
     ...
     * @param longitude 经度
     */
    public Coordinate(CoordinateSystem type, double latitude, double longitude) {
        this.type = type;
     ...
     * @return GC坐标
     */
    public static Coordinate gpsToGcj02(double lat, double lon) {
        ...
        return new Coordinate(CoordinateSystem.GCJ02, mgLat, mgLon);
    }
     ...
     * @return GPS坐标
     */
    public static Coordinate gcj02ToGps(double lat, double lon) {

        double dLat = transformLat(lon - 105.0, lat - 35.0);
        double dLon = transformLon(lon - 105.0, lat - 35.0);
        double m = 1 - E * Math.pow(Math.sin(180D * lat / PI), 2);

        double longitude = lon - (dLon * 180D) / (A * Math.cos(180D * lat / PI) * PI) * Math.sqrt(m);
        double latitude = lat - (dLat * 180D) / (A * (1 - E) * PI) * m * Math.sqrt(m);

        return new Coordinate(CoordinateSystem.GPS, latitude, longitude);
    }
     ...
     * @return 百度坐标
     */
    public static Coordinate gcj02ToBd09(double lat, double lon) {
        ...
        return new Coordinate(CoordinateSystem.BAIDU, bd_lat, bd_lon);
    }
     ...
     * @return GC坐标
     */
    public static Coordinate bd09ToGcj02(double lat, double lon) {
        double x = lon - 0.0065, y = lat - 0.006;
        double z = Math.sqrt(x * x + y * y) - 0.00002 * Math.sin(y * PI);
        double theta = Math.atan2(y, x) - 0.000003 * Math.cos(x * PI);
        double gg_lon = z * Math.cos(theta);
        double gg_lat = z * Math.sin(theta);
        return new Coordinate(CoordinateSystem.GCJ02, gg_lat, gg_lon);
    }
     ...
     * @return GPS坐标
     */
    public static Coordinate bd09ToGps(double lat, double lon) {

        Coordinate gcj02 = bd09ToGcj02(lat, lon);
        return gcj02ToGps(gcj02.getLatitude(), gcj02.getLongitude());

    }

    private static double transformLat(double lon, double lat) {
        return -100D + 0.2D * lon + 3D * lat + 0.2D * lat * lat + 0.1 * lon * lat + 0.2 * Math.sqrt(Math.abs(lon))
            + 40D * (Math.sin(6D * PI * lon) + Math.sin(2 * PI * lon)) / 3D
            + 40D * (Math.sin(PI * lat) + 2D * Math.sin(PI / 3D * lat)) / 3D
            + 320D * (Math.sin(PI / 12D * lat) + 2D * Math.sin(PI / 30D * lat)) / 3D;
    }

    private static double transformLon(double lon, double lat) {
        return 300D + lon + 2.0D * lat + 0.1D * lon * lon + 0.1D * lon * lat + 0.1D * Math.sqrt(Math.abs(lon))
            + 40D * (Math.sin(6D * PI * lon) + Math.sin(2D * PI * lon)) / 3D
            + 40D * (Math.sin(PI * lon) + 2D * Math.sin(PI / 3D * lon)) / 3D
            + 300D * (Math.sin(PI / 12D * lon) + Math.sin(PI / 30D * lon)) / 3D;
    }
