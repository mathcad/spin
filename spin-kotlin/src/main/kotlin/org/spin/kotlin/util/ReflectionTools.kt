...
fun getClassPropertyValueByName(classObj: Any, propertyName: String): Any? =
    classObj::class.java.run {
        this.fields.firstOrNull { it.name == propertyName }?.apply { isAccessible = true }?.get(classObj)
            ?: this.declaredFields.firstOrNull { it.name == propertyName }?.apply { isAccessible = true }?.get(classObj)
...
fun invokeTopMethodByMethodName(otherCallableReference: CallableReference, methodName: String, vararg methodArgs: Any?): Any? {
    ...
    throw IllegalArgumentException("Can't find the method named :$methodName with args ${methodArgs.toList()} in the same file with ${otherCallableReference.name}")
